...
?>
<div class="page-head">
    <div><h1>Tournament Schedules</h1><p>Official master schedule for match times, locations, rounds, and team movement.</p></div>
...
<?php foreach ($schedules as $schedule): ?>
<?php $selectedTeamIds = array_values(array_filter([(int) ($schedule['team_a_id'] ?? 0), (int) ($schedule['team_b_id'] ?? 0)])); ?>
<dialog id="schedule-edit-<?= (int) $schedule['id'] ?>">
    <form method="post" action="<?= site_url('schedules/' . $schedule['id'] . '/update') ?>" class="modal-card wide" data-schedule-team-form data-original-status="<?= esc($schedule['status'], 'attr') ?>" data-confirm-status-change="Change this schedule status? This can affect result-entry availability and the event workflow.">
        <?= csrf_field() ?>
        ...
        <label>Sport<select name="sport_id" required data-schedule-sport><?php foreach ($sports as $sport): ?><option value="<?= (int) $sport['id'] ?>" data-result-type="<?= esc($sport['result_type'], 'attr') ?>" <?= (int) $schedule['sport_id'] === (int) $sport['id'] ? 'selected' : '' ?>><?= esc($sport['name'] . ' · ' . $sport['category']) ?></option><?php endforeach; ?></select></label>
        <div class="form-grid">
        ...
        <div class="sport-checks bracket-team-checks" data-schedule-team-checks><?php foreach ($teams as $team): ?><label class="check"><input type="checkbox" name="team_ids[]" value="<?= (int) $team['id'] ?>" <?= in_array((int) $team['id'], $selectedTeamIds, true) ? 'checked' : '' ?>> <?= esc($team['name']) ?></label><?php endforeach; ?></div>
        <p class="form-note" data-schedule-team-note></p>
        ...
        <button class="btn primary full" data-schedule-save><?= ui_icon('save') ?><span>Save Changes</span></button>
    </form>
</dialog>
<?php endforeach; ?>

<dialog id="schedule-modal">
    <form method="post" action="<?= site_url('schedules') ?>" class="modal-card wide" data-schedule-team-form>
        <?= csrf_field() ?>
        <div class="modal-head"><h2>Add Team Schedule</h2><button type="button" data-close aria-label="Close"><?= ui_icon('x') ?></button></div>
        <label>Sport<select name="sport_id" required data-schedule-sport><option value="">Select sport</option><?php foreach ($sports as $sport): ?><option value="<?= (int) $sport['id'] ?>" data-result-type="<?= esc($sport['result_type'], 'attr') ?>"><?= esc($sport['name'] . ' · ' . $sport['category']) ?></option><?php endforeach; ?></select></label>
        <div class="form-grid">
        ...
        <span class="field-label">Participating Teams</span>
        <div class="sport-checks bracket-team-checks" data-schedule-team-checks><?php foreach ($teams as $team): ?><label class="check"><input type="checkbox" name="team_ids[]" value="<?= (int) $team['id'] ?>"> <?= esc($team['name']) ?></label><?php endforeach; ?></div>
        <p class="form-note" data-schedule-team-note>Select a sport to set the participating-team rule.</p>
        ...
        <p class="form-note">Use Bracket Management to generate linked elimination matches automatically.</p>
        <button class="btn primary full" data-schedule-save><?= ui_icon('save') ?><span>Save Schedule</span></button>
    </form>
</dialog>
<?= $this->endSection() ?>
