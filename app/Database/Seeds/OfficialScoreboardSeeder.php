...
class OfficialScoreboardSeeder extends Seeder
{
    ...
    public function run()
    {
        if (ENVIRONMENT === 'production') {
            throw new RuntimeException('Official scoreboard demo data must not be loaded in production.');
        }

        $this->assertSchemaReady();

        if (! $this->db->transBegin()) {
            throw new RuntimeException('Unable to start official scoreboard seed transaction.');
        }
        ...
        try {
            ...
            $this->ensureResultEntry($resultId, $teams['CBA'], 21, [21], 1, 10);
            $this->ensureResultEntry($resultId, $teams['CCS-CAF'], 17, [17], 2, 7);
            $this->removeUnexpectedResultEntries($resultId, array_values($teams));

            if (! $this->db->transComplete()) {
                throw new RuntimeException('Unable to complete official scoreboard seed transaction.');
            }
        } catch (Throwable $e) {
        ...
        if ($this->changes === 0) {
            CLI::write('ℹ️  No pending official scoreboard seed data; demo data is already seeded.', 'green');
            return;
        }

        CLI::write('✅ Official scoreboard demo data synchronized (' . $this->changes . ' change' . ($this->changes === 1 ? '' : 's') . ').', 'green');
    }
    ...
    private function assertSchemaReady(): void
    {
        foreach (['users', 'events', 'teams', 'locations', 'sport_categories', 'sports', 'schedules', 'weighted_points', 'results', 'result_entries'] as $table) {
            if (! $this->db->tableExists($table)) {
                throw new RuntimeException('Database schema is not ready. Run "php spark migrate" before seeding the official scoreboard.');
            }
        }
        ...
        if (! $this->db->fieldExists('is_active', 'locations')
            ...
            throw new RuntimeException('Scoreboard scoring schema is incomplete. Run "php spark migrate" before seeding the official scoreboard.');
        }
    }
